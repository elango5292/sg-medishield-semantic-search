...
class TableEnricher(BaseEnricher):
    """
    Enrich tables with title and column descriptions.
    
    Uses multimodal LLM to analyze table images.
    """
    ...
    def enrich(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for table in data.get("tables", []):
            enriched_table = self._enrich_table(table)
        ...
        try:
            ...
            message = HumanMessage(
                content=[
                    {
                        "type": "text",
                        "text": """Analyze this table image and provide:
...
Respond in JSON format only:
{
    ...
    "column_descriptions": {"col1": "description", ...}
}"""
                    },
                    {
                        "type": "image_url",
